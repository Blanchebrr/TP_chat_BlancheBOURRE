...
public class ServerUI extends Application implements EventHandler {
    ...
    public void startServer() {
        try {
            // Créé une instance de Server et démarrez-la
            server = new Server();
            server.start();

            // Met à jour l'IHM pour refléter l'état du serveur
            setRunningState();
            sendLogToUI("Serveur démarré sur l'adresse IP : " + ip.getText() + ", port : " + port.getText());
        } catch (IOException e) {
            e.printStackTrace();
            sendLogToUI("Erreur lors du démarrage du serveur : " + e.getMessage());
        }
    }
